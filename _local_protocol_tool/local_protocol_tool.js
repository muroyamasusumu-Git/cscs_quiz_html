...
// ============================================================
          ...
          if (!res.ok) {
            ...
            try {
              ...
              if (ct.indexOf("application/json") !== -1) {
                ...
                detail = String((j && (j.error || j.message)) || "").trim();
              } else {
            ...
            const msg = "元JS取得失敗: HTTP " + String(res.status) + (detail ? (" / " + detail) : "");
            setHistStatus(msg);
  ...
  async function doSplit() {
    ...
    // 追加: HTTP 200 でも ok:false（プロトコル禁止/バリデーション失敗等）はここで止める
    if (result && typeof result === "object" && result.ok === false) {
      const err = String(result.error || result.message || "blocked").trim();
      setStatus("分割不可（プロトコル/検証で停止）: " + (err ? err : "(no details)"));
      return;
    }
