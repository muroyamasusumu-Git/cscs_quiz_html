...
(function () {
  ...
  function refreshAndSend(box, options) {
    ...
    fetchState()
      .then(function (state) {
        ...
        // ★ 何をしているか:
        //   /api/sync/state の結果を view専任のグローバルへ保存する
        //   - 互換のため __cscs_sync_state も維持
        try {
          window.__cscs_sync_state = state;
        } catch (_e) {}
        try {
          window.CSCS_SYNC_DATA = state;
        } catch (_e2) {}
  ...
  function init() {
    ...
    function append() {
      ...
      // ★ 何をしているか:
      //   view専任化：初期表示は state 取得→表示更新のみ（/api/sync/merge は叩かない）
  ...
  // ★ 何をしているか:
  //   view専任化：merge送信は b_sync_merge.js が担当するため、
  //   このファイルでは recordStreak3TodayUnique を上書きしない。
  //   - 既に定義済みなら何もしない（b_sync_merge.js を優先）
  //   - 未定義なら「未実装」を明確化するスタブのみ置く
  if (typeof window.CSCS_SYNC.recordStreak3TodayUnique !== "function") {
    window.CSCS_SYNC.recordStreak3TodayUnique = async function () {
      console.warn("[SYNC-B:view] recordStreak3TodayUnique is not available (b_sync_merge.js not loaded?)");
    };
  }

  // ★ 何をしているか:
  //   view専任化：merge送信は b_sync_merge.js が担当するため、
  //   このファイルでは recordStreak3WrongTodayUnique を上書きしない。
  //   - 既に定義済みなら何もしない（b_sync_merge.js を優先）
  //   - 未定義なら「未実装」を明確化するスタブのみ置く
  if (typeof window.CSCS_SYNC.recordStreak3WrongTodayUnique !== "function") {
    window.CSCS_SYNC.recordStreak3WrongTodayUnique = async function () {
      console.warn("[SYNC-B:view] recordStreak3WrongTodayUnique is not available (b_sync_merge.js not loaded?)");
    };
  }
  ...
  window.addEventListener("online", function () {
    ...
    // ★ 何をしているか:
    //   view専任化：online 復帰時は state 取得→表示更新のみ（/api/sync/merge は叩かない）
    refreshAndSend(box, { suppressDiffSend: true });
  });
