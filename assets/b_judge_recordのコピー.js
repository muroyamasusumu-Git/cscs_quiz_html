...
(function(){
  // …（ここに前回の修正版スクリプト全文をそのまま貼り付け）…
    (function(){
      ...
      // ============================================================
      // ★ 追加: 計測処理が「すべて」完了した合図を1回だけ出す
      //   - tokenなし / ODOA on_nocount / VERIFY on で return した場合はここに来ない
      //   - b_sync_merge.js / cscs_sync_view_b.js がこのイベントを起点に動ける
      // ============================================================
      try{
        window.dispatchEvent(new CustomEvent("cscs:judge:done", {
          detail: {
            qid: qid,
            dayPlay: dayPlay,
            isCorrect: isCorrect,
        ...
        try{
          console.warn("[B:judge] failed to dispatch cscs:judge:done", _eJudgeDone);
        }catch(_){}
      }
    })();
})();