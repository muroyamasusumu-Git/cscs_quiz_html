...
(function () {
  ...
  function bindScaleToElement(el) {
    ...
    el.addEventListener("click", function () {
      ...
      (function shrinkOtherChoicesInSameList() {
        try {
          var body = document.body;
          if (!body || !body.classList || !body.classList.contains("mode-a")) {
            return;
          ...
          // 自分が属している <li> と、その親の <ol class="opts"> を特定する
          var liNode = null;
          try {
            if (typeof el.closest === "function") {
              liNode = el.closest("li");
          ...
          // 同じリスト内のすべての <a> を列挙し、
          // クリックされた要素以外に対して scale(1.0 → 0.95) のアニメーションをかける。
          var anchorList = listNode.querySelectorAll("a");
          for (var i = 0; i < anchorList.length; i++) {
            var other = anchorList[i];
            if (other === el) {
              continue;
            }
            // from は 1.0 固定とし、「通常サイズ → 0.95」への収縮だけを表現する。
            animateScale(other, 1.0, 0.95, 140, easeInOutQuad, null);
          }
        } catch (_eShrink) {
          // アニメ適用で例外が出ても他の処理には影響させない
        }
      })();
