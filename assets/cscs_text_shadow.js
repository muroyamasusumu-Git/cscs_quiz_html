...
(function () {
  ...
  function buildShadowCSS(intensity01, modeName) {
    ...
    // 影をもう少し強めるため、メイン影/外側影の alpha を追加で増やす
    var a1 = (0.22 + 0.30 * i).toFixed(3);  // 0.22..0.52
    var a2 = (0.10 + 0.18 * i).toFixed(3);  // 0.10..0.28

    // modeで質感を変える（crisp はちょい小さく/硬く）
    if (modeName === "crisp") {
      y = (0.4 + 0.6 * i).toFixed(2);       // 0.4..1.0
      blur = (0.8 + 1.8 * i).toFixed(2);    // 0.8..2.6
      // crisp は輪郭が出やすい一方で薄いと負けやすいので、alpha をもう少し上乗せ
      a1 = (0.24 + 0.32 * i).toFixed(3);    // 0.24..0.56
      a2 = (0.12 + 0.20 * i).toFixed(3);    // 0.12..0.32
    }

    // 2段重ね（メインの影 + ごく薄い外側）で “滲み” を抑えつつ読みやすく
    var shadow = [
      "0 " + y + "px " + blur + "px rgba(0,0,0," + a1 + ")",
      // 外側の薄い影の広がりを少し増やして、背景に負けにくくする
