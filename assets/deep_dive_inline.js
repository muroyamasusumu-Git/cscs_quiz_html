  ...
  async function generateDeepDive(section, context){
    ...
    if(hasApiKey()){
      try{
        ...
        return { text: "生成に失敗しました。時間をおいて再度お試しください。", model: "gemini", source: "api", ts: nowTs() };
      }
    } else {
      return { text: "API未設定のため、この場での生成はできません（ウィンドウ版/ローカルがあればそちらに自動フォールバック）。", model: "none", source: "none", ts: nowTs() };
    }
  }
...
`;
  ...
  async function callGemini(prompt){
    ...
    // deep_dive.js に揃える：モデルは 2.5-flash、エンドポイントは v1beta → v1 フォールバック
