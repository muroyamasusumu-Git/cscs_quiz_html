 ...
 *
 * 【重要事項：キー対応表＝契約書（Source of Truth）】
 * この「キー対応表」は、以下 3 点の対応関係を **1 枚に束ねた正式な契約書** である：
 *   1) localStorage のキー名（クライアント側の物理キー）
 *   2) SYNC state のフィールド名（KV に保存される論理構造）
 *   3) delta payload のキー名（merge.ts が受け取る入力インターフェース）
 *
 * - merge.ts / state.ts / クライアント（A/B）の実装は、必ずこの表と一致している必要がある。
 * - この表に無いキーは「正式な同期仕様として存在しない」ものとして扱う。
 *
